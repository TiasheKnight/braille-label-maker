...
def generate_label(objects, speech_text):
    ...
    if objects:
        prompt = f"return a one to three word label to generalize the following:{', '.join(objects)}."
    elif speech_text:
        prompt = f"return a one to three word label that summarises the following: {speech_text}."
    else:
